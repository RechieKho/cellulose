...
| 6 | Ambient occlusion | Consumer's concern. If built into the mesher, it is an **opt-in** flag with AO folded into the merge key (accepting fewer merges), never default. | **done** — `MeshOptions{ .ambient_occlusion = true }`; 0fps corner AO, uniform-AO runs still merge, non-uniform faces emit 1×1; `MeshVertex::occlusion`; level-0 only |
