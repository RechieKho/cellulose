...
| 3 | Seqlock memory model | Keep the benign-race fast path as default; add a strict `std::atomic_ref` element path selectable at compile time (auto-on under `__SANITIZE_THREAD__`). Gate on a microbenchmark of the meshing hot path first. | later (benchmark) |
| 4 | CAS single-writer seqlock | Keep the per-tier `std::mutex`. When a profile shows it hot, fold the writer claim into the sequence counter's low bit (CAS `even → odd`). Also saves ~160 B/chunk. | later (profile) |
| 5 | World directory scaling | Keep the single `std::shared_mutex`. Shard by `ChunkPosition` hash bits only if a profile shows contention. | later (profile) |
