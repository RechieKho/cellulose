...
| BD2 | Build | `option(CELLULOSE_BUILD_BENCHMARKS OFF)`; `cellulose_benchmarks` exe built only when ON. **Release only** — the harness `main` aborts if built without `NDEBUG`. The benchmark build also sets `-DCELLULOSE_BUILD_DEMO=OFF` (a new option, ON by default) so it skips the raylib fetch entirely. | Benchmarks are noise in a Debug build; the renderer is irrelevant to them and its X11 deps aren't installed on the benchmark CI runner. |
