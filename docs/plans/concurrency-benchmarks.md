...
| BD3 | Seqlock instrumentation | Add `option(CELLULOSE_SEQLOCK_STATS OFF)` → when ON, `SeqLock::read` bumps a per-object `std::atomic<u64> m_retries` (relaxed) on each retry; `retries()` accessor. Off = zero cost (the member isn't even declared). | Retry rate is the key seqlock health metric and can't be observed from outside the loop. |
| BD4 | `atomic_ref` variant for D3 | Implement D3's `CELLULOSE_STRICT_ATOMICS` flag now (per-element `std::atomic_ref` in `read_hot` / `write_hot` for `sizeof(HotType) <= 8`), so B2 can A/B the same binary shape. | D3 needs this anyway; building it here means the benchmark measures the real thing. |
